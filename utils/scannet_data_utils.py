...
class ScanNetData(object):
    ...
    def get_infos(self, num_workers=4, has_label=True, sample_id_list=None):
        ...
        def process_single_scene(sample_idx):
            ...
            # Keep the raw cloud above for backward compatibility and expose
            # an aligned copy for axis-aligned boxes and surface keypoints.
            aligned_points = points.copy()
            aligned_points[:, :3] = (
                points[:, :3] @ axis_align_matrix[:3, :3].T
                + axis_align_matrix[:3, 3])
            mmengine.mkdir_or_exist(
                osp.join(self.root_dir, 'points_aligned'))
            aligned_points.tofile(
                osp.join(self.root_dir, 'points_aligned', f'{sample_idx}.bin'))
            info['aligned_pts_path'] = osp.join(
                'points_aligned', f'{sample_idx}.bin')
