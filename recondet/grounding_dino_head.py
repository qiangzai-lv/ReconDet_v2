...
def compute_scene_local_supcon_loss(
    ...
    if len(embeddings) == 0:
        return dict(
            ...
            negative_pair_count=zero.detach(),
    ...
    return dict(
        ...
        negative_pair_count=negative_mask.sum().float().detach(),
...
class ReconGroundingDINOHead(GroundingDINOHead):
    ...
    def forward(self, hidden_states: Tensor, references: List[Tensor],
        ...
        self._last_text_token_mask = text_token_mask.detach()
        return outputs
    ...
    @torch.no_grad()
    def compute_instance_similarity_stats(self, batch_data_samples,
                                          num_views):
        if num_views <= 0 or len(batch_data_samples) % num_views != 0:
            raise ValueError(
                'flattened samples must be divisible by num_views')
        num_queries = self._last_instance_embeddings.shape[1]
        cls_scores, bbox_preds = select_final_matching_outputs(
            self._last_cls_scores, self._last_bbox_preds,
            num_queries)
        query_embeddings = self._last_instance_embeddings[
            :, -num_queries:]
        cfg = self.instance_consistency_cfg
        scene_stats = []
        for start in range(0, len(batch_data_samples), num_views):
            end = start + num_views
            samples = batch_data_samples[start:end]
            gt_instances = [sample.gt_instances for sample in samples]
            img_metas = [sample.metainfo for sample in samples]
            for offset, instances in enumerate(gt_instances):
                text_mask = self._last_text_token_mask[start + offset]
                instances.text_token_mask = text_mask.unsqueeze(0).repeat(
                    len(instances), 1).to(instances.labels.device)
            matches = match_reconstruction_queries(
                self.assigner, cls_scores[start:end], bbox_preds[start:end],
                gt_instances, img_metas)
            background_indices = sample_unmatched_background_queries(
                bbox_preds[start:end], gt_instances, matches, img_metas,
                max_iou=cfg['background_max_iou'],
                ratio=cfg['background_ratio'],
                min_samples=cfg['min_background'],
                max_samples=cfg['max_background'],
                randomize=False)
            result = compute_matched_instance_consistency_loss(
                query_embeddings[start:end], gt_instances, matches,
                background_indices, samples,
                temperature=cfg['temperature'])
            positive_count = result['positive_pair_count']
            negative_count = result['negative_pair_count']
            scene_stats.append({
                'positive_similarity_sum': float(
                    result['positive_similarity'] * positive_count),
                'positive_pair_count': int(positive_count),
                'negative_similarity_sum': float(
                    result['negative_similarity'] * negative_count),
