...
if [[ "${SPLIT}" == "train" || "${SPLIT}" == "all" ]]; then
  merge_split \
    ...
    "${OUTPUT_ROOT}/keypoints_bbox_train.json" \
    "${OUTPUT_ROOT}/keypoints_bbox_train_rejections.json"
fi

if [[ "${SPLIT}" == "test" || "${SPLIT}" == "all" ]]; then
  merge_split \
    "${OUTPUT_ROOT}/keypoints_bbox_val_scenes" \
    "${OUTPUT_ROOT}/keypoints_bbox_val.json" \
    "${OUTPUT_ROOT}/keypoints_bbox_val_rejections.json"
fi
